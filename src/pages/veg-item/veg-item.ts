...
/**
 * Generated class for the VegItemPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-veg-item',
  templateUrl: 'veg-item.html',
})
export class VegItemPage {
  //cid: string = '';
  cid: string = '';
  items: any;
  errorMessage: string;
  ...
  getVegItem() {
    //this.cid = this.navParams.get('categoryId');
    this.rest.getVegItems(this.cid)
      .subscribe(
  ...
  addToCart(item) {
    if(this.email){
      ...
      var cartdata = JSON.stringify({email: this.email,itemId: item.itemid, itemName: item.item_name + this.category, quantity: 1, price: item.price});
